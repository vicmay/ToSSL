...
proc test_hmac_basic {} {
    ...
    puts "DEBUG: calling: ::tossl::hmac -alg sha256 -key \$key \$data"
    set result [catch {::tossl::hmac -alg sha256 -key $key $data} out]
    if {$result != 0} {
...
proc test_hmac_edge_cases {} {
    ...
    if {[string length $out] != 64} {
        puts "FAIL: hmac edge case output length unexpected: [string length $out] (expected 64 hex chars)"
        puts "Output: $out"
        return 1
    }
    puts "PASS: hmac edge case empty input works correctly: $out"
    return 0
}
