...
# Test script for ::tossl::ssl::get_peer_cert command
package require tossl

puts "Testing ::tossl::ssl::get_peer_cert command..."
...
puts "\n=== Test 2: Error handling - invalid connection ==="
set result [catch {
    tossl::ssl::get_peer_cert -conn "nonexistent_conn"
} err]
...
puts "\n=== Test 4: Error handling - missing required parameters ==="
set result [catch {
    tossl::ssl::get_peer_cert -invalid_param "value"
} err]
...
puts "\n=== Test 6: Parameter validation - whitespace in connection name ==="
set result [catch {
    tossl::ssl::get_peer_cert -conn "  test_conn  "
} err]
...
set result [catch {
    ...
    tossl::ssl::get_peer_cert -conn $long_name
} err]
...
puts "\n=== Test 13: Edge case - connection name with null bytes ==="
set result [catch {
    tossl::ssl::get_peer_cert -conn "test\x00conn"
} err]
...
puts "\n=== Test 14: Edge case - connection name with unicode characters ==="
set result [catch {
    tossl::ssl::get_peer_cert -conn "test_conn_🚀_🌍"
} err]
...
set result [catch {
    ...
    puts "✓ Completed 5 rapid get_peer_cert attempts"
} err]
...
puts "\n=== Test 18: Integration with other SSL commands ==="
set result [catch {
    # Test that get_peer_cert can work with contexts created by other commands
    set ctx [tossl::ssl::context create -cert $cert_file -key $key_file]
...
set result [catch {
    ...
    # Test error handling
    if {[catch {
        tossl::ssl::get_peer_cert -conn "nonexistent"
    } err]} {
        if {![string match "*SSL connection not found*" $err]} {
...
puts "\n=== SSL Get Peer Cert Test Summary ==="
puts "All tests completed for ::tossl::ssl::get_peer_cert command"
