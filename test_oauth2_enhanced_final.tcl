...
# Final test suite for enhanced OAuth2 commands with OIDC awareness
...
# Test 4: URL encoding of special characters
...
# Test 5: OIDC authorization URL with complex scope
...
# Test 6: OIDC authorization URL with multiple acr_values
...
# Test 7: OIDC authorization URL with very long parameters
...
# Test 8: OIDC authorization URL with special characters in nonce
...
# Test 9: Comprehensive OIDC flow simulation
...
# Test 11: Error handling for missing OIDC parameters in token refresh
...
# Test 12: Test OIDC authorization URL with only required parameters
test "OIDC authorization URL with only required parameters" {
    tossl::oauth2::authorization_url_oidc \
        -client_id "test_client" \
        -redirect_uri "https://example.com/callback" \
        -scope "openid profile" \
        ...
        -authorization_url "https://accounts.google.com/oauth/authorize" \
        -nonce "test_nonce"
} "https://accounts.google.com/oauth/authorize?response_type=code&client_id=test_client&redirect_uri=https://example.com/callback&scope=openid profile&state=test_state&nonce=test_nonce"

# Test 13: Test OIDC authorization URL with only max_age optional parameter
test "OIDC authorization URL with only max_age optional parameter" {
    tossl::oauth2::authorization_url_oidc \
        -client_id "test_client" \
        -redirect_uri "https://example.com/callback" \
        -scope "openid profile" \
        ...
        -nonce "test_nonce" \
        -max_age "1800"
} "https://accounts.google.com/oauth/authorize?response_type=code&client_id=test_client&redirect_uri=https://example.com/callback&scope=openid profile&state=test_state&nonce=test_nonce&max_age=1800"

# Test 14: Test OIDC authorization URL with only acr_values optional parameter
test "OIDC authorization URL with only acr_values optional parameter" {
    tossl::oauth2::authorization_url_oidc \
        -client_id "test_client" \
        -redirect_uri "https://example.com/callback" \
        -scope "openid profile" \
...
} "https://accounts.google.com/oauth/authorize?response_type=code&client_id=test_client&redirect_uri=https://example.com/callback&scope=openid profile&state=test_state&nonce=test_nonce&acr_values=urn:mace:incommon:iap:gold"
